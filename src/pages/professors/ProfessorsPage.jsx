...
import {
  GraduationCap, Search, Plus, Edit2, Trash2, X,
  Building2, Users, AlertCircle, ChevronDown,
} from "lucide-react";
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <div className="h-[3px] w-full bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-500" />
...
/* ── Professor Card ─────────────────────────────────────────────────────── */
function ProfessorCard({ professor, onEdit, onDelete, index }) {
  const [from, to] = avatarGradient(professor.name);
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white border border-slate-200/70 rounded-2xl overflow-hidden
                 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-50/60 transition-all duration-200"
    >
      {/* Top hover accent */}
      ...
      <div className="p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className={`relative flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br ${from} ${to}
            ...
            <div className="absolute -bottom-0.5 -right-0.5 w-[18px] h-[18px] rounded-full bg-white
              ...
              <GraduationCap size={9} className="text-emerald-600" />
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-xs text-slate-500 truncate font-medium">
                {professor.department_name || "No department"}
              </span>
            </div>
          </div>
        ...
        {professor.department_name ? (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full
                           bg-emerald-50 border border-emerald-100 text-emerald-700 text-[11px] font-semibold">
            <Building2 size={9} />
            {professor.department_name}
          </span>
        ) : (
    ...
    >
      ...
      <div className="text-center max-w-xs">
        <p className="font-bold text-slate-700 text-[15px]">
          {isFiltered ? "No professors found" : "No professors yet"}
        </p>
        <p className="text-sm text-slate-400 mt-1.5 leading-relaxed">
          {isFiltered
            ? "Try adjusting your search or clearing the filter."
            : "Add your first professor to start building your faculty directory."}
        </p>
      </div>
      {!isFiltered && (
        <button
          onClick={onAdd}
          className="mt-1 flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-white
                     text-sm font-semibold hover:bg-emerald-700 active:scale-95 transition-all
...
export default function ProfessorsPage() {
  ...
  }, [professors, query, sortBy]);

  const deptCount = useMemo(
    () => new Set(professors.map((p) => p.department_name).filter(Boolean)).size,
    [professors]
  );
  ...
  const handleSubmit = async (data) => {
    ...
    const result = editData
      ? await editProfessor(editData.id, data)
      : await addProfessor(data);
    setSaving(false);
    if (result.success) {
      toast.success(editData ? "Professor updated!" : "Professor added!");
      closeModal();
    } else {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Remove this professor? This cannot be undone.")) return;
    const result = await removeProfessor(id);
    result.success ? toast.success("Professor removed.") : toast.error(result.message);
  };

  return (
    <div className="min-h-full bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 p-6 lg:p-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Faculty</h1>
            </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white
                       text-sm font-bold hover:bg-emerald-700 active:scale-95 transition-all
                       shadow-md shadow-emerald-200 whitespace-nowrap"
          >
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-2.5 text-sm bg-white border border-slate-200 rounded-xl
                       ...
                       focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-300 transition-all"
          />
          <AnimatePresence>
            {query && (
              <motion.button
      ...
      <AnimatePresence>
        {query && !loading && (
          ...
          >
            {filtered.length} of {professors.length} results for "{query}"
          </motion.p>
        )}
      </AnimatePresence>
      ...
      {loading && (
        <div className="flex flex-col items-center gap-3 py-24">
          ...
          <p className="text-sm text-slate-400 font-medium">Loading faculty…</p>
        </div>
      )}
      ...
      <Modal
        isOpen={showModal}
        onClose={closeModal}
        title={editData ? "Edit Professor" : "Add Professor"}
      >
        <ProfessorForm
